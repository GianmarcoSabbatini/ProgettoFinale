...
app.post(
  ...
  [
    ...
    body('password')
      ...
      .matches(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/)
      .withMessage(
        'Password deve contenere almeno 8 caratteri, una maiuscola, una minuscola e un numero'
      ),
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      // Trova utente
      const [users] = await db.execute('SELECT id, username, email FROM users WHERE email = ?', [
        email,
      ]);
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      if (resets.length === 0) {
        logger.warn('Token reset password non valido o scaduto', {
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      await db.execute('INSERT INTO messages (title, content, author) VALUES (?, ?, ?)', [
        title,
        content,
        author,
      ]);
...
app.put(
  '/api/messages/:id',
  verifyToken,
  [body('content').trim().notEmpty(), body('author').trim().notEmpty()],
  async (req, res) => {
    try {
      // Validazione
      const errors = validationResult(req);
...
app.delete('/api/timesheet/:id', verifyToken, async (req, res) => {
  try {
    ...
    // Verifica proprietà
    const [entries] = await db.execute('SELECT user_id FROM timesheet WHERE id = ?', [id]);
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const userId = req.user.userId;
      const { month, year, gross_amount, net_amount, issue_date, issued_by, salary_details } =
        req.body;
...
app.post(
  ...
  verifyToken,
  [body('month').trim().notEmpty(), body('year').isInt({ min: 2000, max: 2100 })],
  async (req, res) => {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
      ...
      const [result] = await db.execute(
        ...
        [
          ...
          netAmount.toFixed(2),
          issueDateStr,
          'CoreTeam Digital - Sistema Automatico',
