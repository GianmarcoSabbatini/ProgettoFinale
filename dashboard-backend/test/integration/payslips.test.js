...
describe('Payslips API', function () {
  ...
  before(async function () {
    ...
    await db.execute('INSERT INTO profiles (user_id, nome, cognome) VALUES (?, ?, ?)', [
      testUserId,
      'Payslip',
      'Test',
    ]);
    ...
    // Login to get token
    const loginRes = await request(BASE_URL).post('/api/login').send({
  ...
  describe('POST /api/payslips/generate', () => {
    it('should generate payslip from timesheet entries', async () => {
      ...
      await db.execute(
        ...
        [testUserId, '2025-11-01', 8, 'Development work', 25.0]
      );
      await db.execute(
        'INSERT INTO timesheet_entries (user_id, date, hours, description, hourly_rate) VALUES (?, ?, ?, ?, ?)',
        [testUserId, '2025-11-02', 7.5, 'Testing', 25.0]
      );
    ...
    it('should calculate correct net amount (30% deductions)', async () => {
      ...
      await db.execute(
        'INSERT INTO timesheet_entries (user_id, date, hours, description, hourly_rate) VALUES (?, ?, ?, ?, ?)',
        [testUserId, '2025-11-01', 10, 'Work', 20.0]
      );
      ...
      expect(grossAmount).to.equal(200.0); // 10 * 20
      expect(netAmount).to.equal(140.0); // 200 * 0.70
    });
    ...
    it('should require authentication', async () => {
      const res = await request(BASE_URL).post('/api/payslips/generate').send({
        userId: testUserId,
        month: 'Novembre',
        year: 2025,
      });
    ...
    it('should handle multiple hourly rates correctly', async () => {
      ...
      await db.execute(
        'INSERT INTO timesheet_entries (user_id, date, hours, description, hourly_rate) VALUES (?, ?, ?, ?, ?)',
        [testUserId, '2025-11-01', 5, 'Junior work', 15.0]
      );
      await db.execute(
        'INSERT INTO timesheet_entries (user_id, date, hours, description, hourly_rate) VALUES (?, ?, ?, ?, ?)',
        [testUserId, '2025-11-02', 5, 'Senior work', 30.0]
      );
      ...
      expect(res.status).to.equal(201);
      const grossAmount = parseFloat(res.body.payslip.gross_amount);
      expect(grossAmount).to.equal(225.0); // (5*15) + (5*30)
    });
  });
  ...
  describe('PUT /api/payslips/:id/recalculate', () => {
    ...
    beforeEach(async function () {
      ...
      await db.execute(
        'INSERT INTO timesheet_entries (user_id, date, hours, description, hourly_rate) VALUES (?, ?, ?, ?, ?)',
        [testUserId, '2025-11-01', 8, 'Work', 20.0]
      );
    ...
    it('should recalculate payslip when timesheet changes', async () => {
      ...
      await db.execute(
        'INSERT INTO timesheet_entries (user_id, date, hours, description, hourly_rate) VALUES (?, ?, ?, ?, ?)',
        [testUserId, '2025-11-03', 4, 'Extra work', 20.0]
      );
    ...
    it('should recalculate with updated timesheet values', async () => {
      ...
      await db.execute('UPDATE timesheet_entries SET hours = 10 WHERE user_id = ? AND date = ?', [
        testUserId,
        '2025-11-01',
      ]);
    ...
    it('should require authentication for recalculation', async () => {
      const res = await request(BASE_URL).put(`/api/payslips/${payslipId}/recalculate`);
  ...
  describe('GET /api/payslips', () => {
    ...
    it('should require authentication', async () => {
      const res = await request(BASE_URL).get('/api/payslips').query({ userId: testUserId });
  ...
  describe('Payslip Calculations - Edge Cases', () => {
    it('should handle decimal hours correctly', async () => {
      await db.execute(
        'INSERT INTO timesheet_entries (user_id, date, hours, description, hourly_rate) VALUES (?, ?, ?, ?, ?)',
        [testUserId, '2025-11-01', 7.75, 'Work', 22.5]
      );
    ...
    it('should handle large numbers of hours', async () => {
      ...
      for (let i = 1; i <= 20; i++) {
        await db.execute(
          'INSERT INTO timesheet_entries (user_id, date, hours, description, hourly_rate) VALUES (?, ?, ?, ?, ?)',
          [testUserId, `2025-11-${String(i).padStart(2, '0')}`, 8, 'Work', 25.0]
        );
      }
      ...
      const grossAmount = parseFloat(res.body.payslip.gross_amount);
      expect(grossAmount).to.equal(4000.0); // 160 * 25
    });
    ...
    it('should handle INPS deduction calculation', async () => {
      ...
      expect(inpsDeduction).to.be.closeTo(grossAmount * 0.1, 0.01); // 10% INPS
    });
    ...
    it('should handle IRPEF deduction calculation', async () => {
      ...
      expect(irpefDeduction).to.be.closeTo(grossAmount * 0.2, 0.01); // 20% IRPEF
    });
  });
});
