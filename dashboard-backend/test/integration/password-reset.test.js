...
describe('Password Reset Flow', function () {
  ...
  describe('POST /api/auth/forgot-password', () => {
    ...
    it('should invalidate old tokens when generating new one', async () => {
      ...
      const [tokens] = await db.execute('SELECT * FROM password_resets WHERE user_id = ?', [
        testUserId,
      ]);
  ...
  describe('POST /api/auth/reset-password', () => {
    ...
    it('should reset password with valid token', async () => {
      ...
      const res = await request(BASE_URL).post('/api/auth/reset-password').send({
        token: validToken,
    ...
    it('should reject used token (one-time use)', async () => {
      ...
      const res1 = await request(BASE_URL).post('/api/auth/reset-password').send({
        token: validToken,
        newPassword: newPassword,
      });
      ...
      const res2 = await request(BASE_URL).post('/api/auth/reset-password').send({
        token: validToken,
