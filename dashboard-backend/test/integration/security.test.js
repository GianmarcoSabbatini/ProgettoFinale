...
describe('Security Features', function () {
  ...
  describe('Security Headers (Helmet)', () => {
    it('should include X-Content-Type-Options header', async () => {
      const res = await request(BASE_URL).get('/api/messages').expect(401); // Non autenticato, ma headers presenti
  ...
  describe('Input Validation', () => {
    ...
    it('should reject XSS attempts in registration', async () => {
      const res = await request(BASE_URL).post('/api/register').send({
